...
void mul(const vector<int>& a, const vector<int>& b, vector<int>& result)
{
	...
	for (size_t i = 0; i < a.size(); i++)
	{
		...
		{
			long long current = result[i + j] + carry + (j < b.size() ? a[i] * b[j] : 0);//если второе число еще есть, то перемножаются числа из одного разряда, иначе прибавляется ноль
...
void div(const vector<int>& a, int ch, vector<int>& result)//организация деления столбиком
{
	result.clear();
